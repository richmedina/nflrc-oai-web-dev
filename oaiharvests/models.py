...
TYPES = ['publisher', 'description.provenance', 'identifier.doi', 'title', 'bitstream', 'date.available', 'type.dcmi', 'relation.uri', 'identifier.citation', 'format.extent', 'description.abstract', 'date.accessioned', 'language.iso', 'relation.ispartofseries', 'identifier.issn', 'date.issued', 'identifier.uri', 'type', 'contributor.author', 'subject', 'volume', 'startingpage', 'llt.topic', 'relation.isbasedon']
...
class Collection(TimeStampedModel):
    ...
    def title_tuple(self):
        ...
        title = {'title': self.name, 'date': '', 'subtitle': '', 'object': self, 'editors': self.editor_list(), 'color_code': 1}
        try:
            ...
            pos = vol_date.lower().find('special ')
            if pos >= 0:
        ...
        for rec in toc_item_list:  # Returns a list of [ [REC OBJ, PAGE START, PAGE END, VOL NUM], ... ]
            ...
            try:
                for rec_type in rec_data['type']:
                    toc_item = [rec_obj, [], [], '', '']  # Set defaults (rec object, authors, abstract, start page, date accessioned)
